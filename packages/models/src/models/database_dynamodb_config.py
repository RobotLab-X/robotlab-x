...
class DatabaseDynamodbConfig(BaseModel):
    ...
    table_prefix: Optional[str] = Field("", description="Prefix for DynamoDB tables", json_schema_extra={"example":"myapp_"})

# managed
    @classmethod
    def from_yaml(cls, yaml_str: str):
        """Load DatabaseDynamodbConfig from a YAML string."""
        data = yaml.safe_load(yaml_str)
        return cls(**data)

    @classmethod
    def from_yaml_file(cls, file_path: str):
        """Load DatabaseDynamodbConfig from a YAML file."""
        with open(file_path, "r") as file:
            return cls.from_yaml(file.read())

    @classmethod
    def from_json(cls, json_str: str):
        """Load DatabaseDynamodbConfig from a JSON string."""
        data = json.loads(json_str)
        return cls(**data)

    @classmethod
    def from_json_file(cls, file_path: str):
        """Load DatabaseDynamodbConfig from a JSON file."""
        with open(file_path, "r") as file:
