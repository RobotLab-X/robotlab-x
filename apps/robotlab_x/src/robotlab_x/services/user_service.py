...
def get_path_user(path: str, user: dict, request: Request):
    logger.info("===============get_user called==============")
    logger.info(f"Received request to retrieve {path} with path: {path}")
    db = get_database_client()
    if db:
        item = db.get_item("user", path)
    return item
...
def update_user(id: str, new_item: User, user: dict, request: Request):
    ...
    db.update_item("user", id, new_item.model_dump(), include_nulls=True)
...
def process_user_request(payload: Dict[str, Any], user: dict, request: Request):
    logger.info("===============process_user_request==============")
    ...
    db = get_database_client()
    if db:
        records = db.query_items(table_name="user", criteria={})
        # do stuff
